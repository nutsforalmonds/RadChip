...
//Added overload for GS Shader
void ShaderController::createVGFShader(const string& shader_name, const char * vert_file, const char * geom_file, const char* frag_file)
{
	shaders[shader_name] = GLSLProgram();
	shaders[shader_name].compileShaderFromFile(vert_file, GLSLShader::VERTEX);
	shaders[shader_name].compileShaderFromFile(geom_file, GLSLShader::GEOMETRY);
	shaders[shader_name].compileShaderFromFile(frag_file, GLSLShader::FRAGMENT);
	shaders[shader_name].link();
}
